...
TAKE_PROFITS = [0.30]   # هدف واحد فقط: +30% — خروج كامل فوري عند تحققه
# استراتيجية "الخروج الكامل" (2026-09-25، تحل محل الجني الجزئي 50%):
# تحليل الأرشيف أثبت أن بيع 50% + وقف التعادل للنصف المتبقي يخنق الأرباح
# (الفوز الصافي +11.6% فقط مقابل خسارة -20% → يتطلب 64% نسبة فوز مستحيلة)،
# بينما الخروج الكامل يعطي +22.4% صافي → عتبة التعادل تنزل لـ47% (ممكنة)
STOP_LOSS = 0.15                    # وقف الخسارة: -15% (كان -25%: الخروج المبكر
                                    # يحمي رأس المال في دورة الميم الحالية)
POSITION_MAX_AGE_H = 30              # time-stop: إغلاق الصفقة بعد 30 ساعة إن لم تصل
                                    # للهدف — تحرير أماكن المحفظة من "الفلوس الميتة"
                                    # (كانت 7 أيام: POSITION_MAX_AGE_DAYS)
...
PAPER_MAX_OPEN = 4            # سقف الصفقات الوهمية المفتوحة في نفس الوقت
CIRCUIT_BREAKER_SL_STREAK = 3 # بعد 3 إغلاقات خاسرة متتالية → إيقاف الشراء
CIRCUIT_BREAKER_HALT_H = 24   # مدة إيقاف الشراء بالساعات
SL_COOLDOWN_H = 48            # منع إعادة الدخول في نفس العملة بعد إغلاق خاسر
# توزيع البيع عند الهدف: 1.0 = خروج كامل فوري (2026-09-25)
# (كان 0.5: بيع 50% + وقف تعادل للباقي — أثبتت البيانات أنه يخنق الربح)
PAPER_SELL_FRACTIONS = (1.0,)
